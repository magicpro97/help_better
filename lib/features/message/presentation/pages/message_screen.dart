...
import 'package:better_help/features/message/presentation/widgets/chat_bar.dart';
import 'package:better_help/features/message/presentation/widgets/message_group_card.dart';
import 'package:better_help/features/message/presentation/widgets/message_item.dart';
...
class _MessageScreenState extends State<MessageScreen>
  ...
  void initState() {
    ...
    messageBloc.add(ComeInEvent(
        messageGroup: widget.messageGroup, currentUser: widget.currentUser));
    // ignore: close_sinks
              ...
              children: <Widget>[
                Expanded(
                  child: messageGroup.memberIds.length > 2
                      ? _buildWithManyOtherUser(messageGroup.id, currentUser.id)
                      : _buildMessageList(
                          messageGroup.id, otherUser, currentUser.id),
                ),
                ChatBar(
                  messageBloc: messageBloc,
  ...
  Widget _buildWithManyOtherUser(String messageGroupId, String currentUserId) =>
      Container();

  /*FutureBuilder<List<User>>(
          future: MessageGroupDao.getOtherUser(
              groupId: messageGroupId, currentUserId: currentUserId),
          builder: (context, otherUserSnapshot) {
            if (otherUserSnapshot.hasError) {
              log(otherUserSnapshot.error.toString(), name: _TAG);
              return Center(
            ...
            if (otherUserSnapshot.data.isEmpty) {
              return Container();
            }
  ...
  StreamBuilder<List<Message>> _buildMessageList(
      String messageGroupId, List<User> otherUsers, String currentUserId) {
    return StreamBuilder<List<Message>>(
        ...
        builder: (context, snapshot) {
          ...
          final lastCurrentUserMessage = messages.firstWhere(
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              if (index < messages.length - 1) {
                if (message.created
                        .difference(messages[index + 1].created)
                        .inMinutes >
                    3) {
                  isFirstMessageGroup = true;
                  if (tempTimeGroupMessage != null &&
                      tempTimeGroupMessage.isNotEmpty) {
                    lastConversationMessage = tempTimeGroupMessage.last;
                  }
              ...
              return MessageItem(
                ...
                isLastCurrentUserMessage:
                    lastCurrentUserMessage?.userId == message?.userId &&
                            lastCurrentUserMessage?.id == message?.id ??
                        false,
                isLastConversationMessage: lastConversationMessage != null,
  ...
  Future<bool> _onWillPop(User currentUser, MessageGroup messageGroup) async {
    messageBloc
        .add(GoOutEvent(messageGroup: messageGroup, currentUser: currentUser));
    return true;
  }
}
